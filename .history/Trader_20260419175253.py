...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        for product, depth in state.order_depths.items():
            ...
            orders, bid_price, ask_price = self._make_market(
                product, fair_value, position, depth, state.timestamp
            )
    ...
    # Maximum number of ticks the OBI signal is allowed to shift the
    # OSMIUM micro_price away from the 10,000 peg. Acts as a hard cap
    # on how much we trust depth imbalance as a directional signal.
    MAX_OBI_SHIFT = 3
    ...
    def _make_market(
        ...
        - OSMIUM: anchors the Vault around an OBI-shifted micro_price
          (10,000 +/- a small bias toward the heavier side of the book).
        - Trending products: passive maker quotes around DEMA fair value.
        """
        if product == self.OSMIUM:
            # --- Order Book Imbalance (OBI) ---
            total_buy_vol = sum(order_depth.buy_orders.values())
            total_sell_vol = sum(abs(v) for v in order_depth.sell_orders.values())
            total_vol = total_buy_vol + total_sell_vol

            if total_vol > 0:
                obi = total_buy_vol / total_vol
            else:
                obi = 0.5  # empty book -> neutral

            # Map obi in [0, 1] to a shift in [-MAX_OBI_SHIFT, +MAX_OBI_SHIFT].
            obi_shift = (obi - 0.5) * 2.0 * self.MAX_OBI_SHIFT
            micro_price = self.OSMIUM_FAIR_VALUE + obi_shift

            print(
                f"[OSMIUM_OBI] T: {timestamp} | "
                f"OBI: {obi:.3f} | "
                f"Shift: {obi_shift:+.2f} | "
                f"Micro: {micro_price:.2f}"
            )

            bid_price, ask_price = self._osmium_quotes(position, micro_price)
        else:
    ...
    def _osmium_quotes(self, position: int, micro_price: float) -> Tuple[int, int]:
        """
        Defensive Peg Maker for ASH_COATED_OSMIUM, anchored to micro_price.

        Same Vault math as before -- inventory shifts the EDGES, both
        floored at 1.0 so we always demand >= 1 tick of profit -- but
        the symmetric center is the OBI-derived micro_price instead of
        the hardcoded 10,000 peg. micro_price is bounded to within
        MAX_OBI_SHIFT ticks of the peg by construction in _make_market.
            ...
            bid_price = floor(micro_price - bid_edge)
            ask_price = ceil (micro_price + ask_edge)
        """
        ...
        bid_price = math.floor(micro_price - bid_edge)
        ask_price = math.ceil(micro_price + ask_edge)
